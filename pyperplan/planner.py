...
def _parse(domain_file, problem_file):
    #set global problemfile to problemfile
    global problem_global
    problem_global = problem_file
    ...
    with open(f"{problem_file}"[:-5] + "_out.csv", 'a') as f:
...
def search_plan(
    ...
    with open(f"{problem_file}"[:-5] + "_out.csv", 'a') as f:
        f.write("{:.2}".format(time.process_time() - search_start_time))
    return solution
