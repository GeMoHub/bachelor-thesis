"""
Euclidean Heuristic scaled by Landmarks, goal-aware
"""
...
def get_distance(fact_list, goals_pos, landmark_cost_pos, min_d, node):
...
def get_distance_to_goal_states(landmark_pos, goals_pos, fact_list):
    """
    ...
    """
    goal_state_pos = np.multiply(np.ones(len(fact_list)), landmark_pos)
...
class EuclideanScaledByLandmarkHeuristic(Heuristic):
    def __init__(self, task):
        ...
        self.min_d = get_distance_to_goal_states(self.landmark_cost_pos, self.goals_pos, self.fact_list)


    def __call__(self, node):
        """Returns the heuristic value for "node"."""
        return get_distance(self.fact_list, self.goals_pos, self.landmark_cost_pos, self.min_d, node)
